...
describe('findSubchartValuesDir', () => {
  ...
  test.it('refering chart.yaml', () => {
    fs.copyFileSync(
      p.join(testRoot(), 'lib', 'fixture', 'chart', 'Chart.yaml'),
      p.join(testRoot(), 'lib', 'testfield', 'chart', 'Chart.yaml'),
    )
  ...
  test.it('specifying the charts', () => {
    cproc.execSync(
      `${binPath()} generate -s dev prod test A B C -- redis postgres`,
      {
        cwd: p.join(testRoot(), 'lib', 'testfield', 'chart'),
      },
    )
...
describe('findUninstalledSubcharts', () => {
  ...
  test.it('all installed', () => {
    expect(findUninstalledSubcharts(['mysql', 'redis'])).to.have.members([])
  })

  test.it('some installed', () => {
    expect(findUninstalledSubcharts(['postgres', 'redis'])).to.have.members([
      'postgres',
    ])
  })

  test.it('uninstalled only', () => {
    expect(findUninstalledSubcharts(['postgres'])).to.have.members([
      'postgres',
    ])
  })
})
...
describe('findInvalidPatchValues', () => {
  ...
  .do(() => {
    shelljs.mkdir(
      '-p',
      p.join(testRoot(), 'lib', 'testfield', 'chart', 'values', 'mysql'),
    )
    shelljs.mkdir(
      '-p',
      p.join(testRoot(), 'lib', 'testfield', 'chart', 'values', 'redis'),
    )
    fs.copyFileSync(
      p.join(testRoot(), 'lib', 'fixture', 'chart', 'Chart.yaml'),
      p.join(testRoot(), 'lib', 'testfield', 'chart', 'Chart.yaml'),
    ...
    cproc.execSync(
      `${binPath()} generate -s dev -s prod -s test -s A -s B -s C`,
      {
        cwd: p.join(testRoot(), 'lib', 'testfield', 'chart'),
      },
    )
  ...
  .it('both([stage].yaml, base.yaml)', () => {
    expect(findInvalidPatchValues(['redis', 'mysql'], 'dev')).to.deep.equal(
      [],
    )
  })
...
describe('groupByStage', () => {
  ...
  .do(() => {
    shelljs.cp(
      '-r',
      p.join(testRoot(), 'lib', 'fixture', 'chart', 'mysql'),
      p.join(testRoot(), 'lib', 'testfield', 'chart', 'values', 'mysql'),
    )
  ...
  .do(() => {
    shelljs.cp(
      '-r',
      p.join(testRoot(), 'lib', 'fixture', 'chart', 'redis'),
      p.join(testRoot(), 'lib', 'testfield', 'chart', 'values', 'redis'),
    )
  ...
  .catch(
    `ENOENT: no such file or directory, scandir '${testRoot()}/lib/testfield/chart/values/postgres'`,
  )
  .it('invalid chart')
})
